...
import { X, Loader2, PackagePlus, Layers } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-3 backdrop-blur-xs sm:p-4">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 sm:px-5 sm:py-4">
          ...
          {/* Chamada opcional para criação em lote */}
          {onOpenBatchCreate && (
            <div className="mt-4 flex items-center justify-between gap-2 rounded-xl bg-indigo-50/70 border border-indigo-100/80 px-3.5 py-2.5 text-xs">
              <div className="flex items-center gap-2 text-slate-700">
                <Layers className="h-4 w-4 text-indigo-600 shrink-0 stroke-[2.2]" />
                <span className="font-medium">Precisa cadastrar vários itens de uma vez?</span>
              </div>
              <button
                type="button"
                id="btn-switch-to-batch-callout"
                onClick={() => {
                  onClose();
                  onOpenBatchCreate();
                }}
                className="cursor-pointer font-bold text-indigo-600 hover:text-indigo-800 hover:underline shrink-0"
              >
                Criar em Lote &rarr;
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 my-3 py-3">
            <div>
              <label
